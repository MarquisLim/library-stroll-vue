<template>
    <AppLayout title="Studio">
        ...
        <div v-if="showCreateModal" class="modal modal-open">
            <div class="modal-box bg-gray-800 text-white">
                ...
                <div class="border-2 border-dashed border-gray-500 p-4 rounded text-center"
                     ...
                     @dragover.prevent
                >
                    <p v-if="!tempFile">Перетащите файл или нажмите для выбора</p>
                    <p v-else>Выбран файл: {{tempFile.name}}</p>
                    <input type="file" ref="createFileInput" class="hidden" @change="onFileChangeCreate" />
                    <button class="btn mt-2 btn-secondary" @click="browseFileCreate">Выбрать файл</button>
                </div>
                <div class="modal-action">
                    <button class="btn" @click="showCreateModal=false">Отмена</button>
                    <button class="btn btn-primary" @click="createArtFromFile" :disabled="!tempFile">Создать</button>
                </div>
            </div>
        </div>
        ...
        <div class="fixed bottom-0 left-0 m-4 space-y-2 w-64 z-50">
            <div v-for="(e,i) in errorMessages" :key="i" class="bg-red-500 text-white px-4 py-2 rounded">{{ e }}</div>
            <div v-if="successMessage && !successMessage.includes('Сохранение')" class="bg-green-500 text-white px-4 py-2 rounded">
                {{ successMessage }}
            </div>
        </div>
