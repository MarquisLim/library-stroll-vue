...
const props = defineProps({ artwork: Object, open: Boolean })
...
async function save() {
    ...
    `/studio/update-draft/${props.artwork.id}`,
    {
            title:           title.value,
...
<template>
    <div v-if="open" class="fixed inset-0 bg-base-content/60 flex items-center justify-center z-50 p-4" @click="onBgClick">
        <div class="bg-base-100 text-base-content w-full max-w-md rounded-lg p-6 space-y-4 overflow-y-auto max-h-[90vh]">
            <h2 class="text-lg font-semibold flex items-center mb-2">
                <i class="mdi mdi-pencil-outline mr-2"></i> Редактирование
            </h2>

            <div class="mb-4">
                <video v-if="artwork.type==='video'" :poster="artwork.thumb_url" :src="artwork.original_url" controls class="w-full rounded"/>
                <img v-else :src="preview || artwork.thumb_url" class="w-full rounded"/>
            </div>

            <div v-if="!artwork.is_published" class="bg-base-200 bg-opacity-40 p-3 rounded border border-base-300 text-base-content text-center space-y-2">
                <input id="fileInput" type="file" class="hidden" accept="image/*,video/*" @change="onFile"/>
                <label for="fileInput" class="btn btn-sm btn-outline">
                    <i class="mdi mdi-upload mr-1"></i>Заменить файл
                </label>
                <img v-if="preview && !isVideoDraft" :src="preview" class="mt-2 max-h-56 object-contain mx-auto rounded"/>
                <div v-else-if="isVideoDraft" class="flex items-center justify-center mt-2 text-base-content/70">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input id="title" v-model="title" class="input input-bordered w-full" placeholder="Введите название"/>
                </div>
                <div>
                    <label for="description" class="block text-sm mb-1 text-base-content/70">Описание</label>
                    <textarea id="description" v-model="description" rows="3" class="textarea textarea-bordered w-full" placeholder="Введите описание"></textarea>
                </div>

                <div class="flex flex-wrap gap-4">
                    <label class="label cursor-pointer gap-2">
                        <input type="checkbox" v-model="is_private" class="checkbox checkbox-sm"/> <span class="label-text">Приватный</span>
                    </label>
                    <label class="label cursor-pointer gap-2">
                        <input type="checkbox" v-model="is_adult" class="checkbox checkbox-sm"/> <span class="label-text">Взрослый контент+</span>
                    </label>
                    <label class="label cursor-pointer gap-2">
                        <input type="checkbox" v-model="has_ai" class="checkbox checkbox-sm"/> <span class="label-text">AI-generated</span>
                    </label>
                    <label class="label cursor-pointer gap-2">
                        <input type="checkbox" v-model="allow_download" class="checkbox checkbox-sm"/> <span class="label-text">Скачивание</span>
                    </label>
                    <label class="label cursor-pointer gap-2">
                        <input type="checkbox" v-model="allow_comments" class="checkbox checkbox-sm"/> <span class="label-text">Комментарии</span>
                    </label>
                </div>
            ...
            <div class="flex justify-end gap-2 pt-4">
                ...
                <button v-if="!artwork.is_published" class="btn btn-success" :disabled="!canSave" @click="publish">
                    Опубликовать
                </button>
            </div>
        </div>
