<template>
    <div class="absolute z-50 p-4 rounded shadow"
         ...
         :style="{ position: 'absolute', top: position.top - 0 + 'px', left: position.left + 'px' }">
        <input v-if="collections && collections.length > 0"  type="text" placeholder="Поиск коллекций..." class="w-full px-2 py-1 rounded border border-gray-600 bg-gray-700 mb-2" @input="filterCollections"/>
        <div v-if="collections && collections.length > 0"  class="max-h-40 overflow-auto mb-2 bg-gray-700 rounded p-1">
        ...
        <button v-if="collections && collections.length > 0"  class="btn btn-primary w-full mb-2" @click="save">Сохранить</button>
