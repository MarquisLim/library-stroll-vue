<template>
    <!-- clickable anchor fixes middle-click / cmd-click and improves SEO -->
    <a
        ref="rootEl"
        :href="`/artworks/${art.id}`"
        class="relative group mb-2 break-inside-avoid block transition-transform duration-300 ease-in-out hover:scale-105"
    ...
    >
        ...
        <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-opacity pointer-events-none" />
        ...
        <!-- ───────────── Mobile footer ───────────── -->
        <div class="block md:hidden bg-black bg-opacity-50 text-white px-2 py-1 rounded-b flex items-center justify-between">
...
/* intersection observer (autoplay on mobile scroll) */
let observer
onMounted(() => {
...
const thumbOrOriginal = computed(() => {
    ...
    if (!m) return null
    return props.art.thumb_url ? `${props.art.thumb_url}?v=${m.updated_at}` : `${m.original_url}?v=${m.updated_at}`
})
const previewSrc = computed(() => previewExists.value ? `${props.art.preview_url}?v=${props.art.media[0].updated_at}` : null)
...
@keyframes like { 0%{transform:scale(1)} 25%{transform:scale(1.2)} 50%{transform:scale(.9)} 75%{transform:scale(1.05)} 100%{transform:scale(1)} }
.animate-like{animation:like .4s ease}
</style>
